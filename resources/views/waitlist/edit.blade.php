<x-admin-layout>
    ...
    <x-form-card action="{{ route('waitlist.update', $visit->id) }}" title="Create New Visit" method="PUT">
        <x-slot name="body">
                    ...
                    <div class="flex space-x-2">
                        ...
                        <div class="text-center font-semibold text-gray-600">
                            <x-select id="day" name="day">
                                @for ($i = 1; $i <= 30; $i++) <option value={{ $i }} @if ($i==$day) selected @endif>
                                    {{ $i }}</option>
                                    @endfor
                            </x-select>
                ...
                <div class="mt-1 sm:mt-0 sm:col-span-2">
                    <div class=" w-1/3">
                        <x-multi-checkbox name="has_car" value="1" title="Check If Visitor Has Car"
                            checked="{{ ($visit->has_car === 1 ?? '') ? true : false }}" onclick="ShowHideDiv(this)" />
                    </div>

                    <div id="dvHasCar" class="mt-1 w-1/2"
                        style="display: {{ $visit->has_car === 1 ? 'block' : 'none' }}"">
                        <x-input type=" text" name="plates" value="{{ $visit->plates }}" placeholder="AA B12345" />
